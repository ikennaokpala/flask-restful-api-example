...
from src.tests.support.factories import ProjectFactory
...
class TestUploadMZXMLFileAndAssociateWithDataType(BaseTestCase):
    def setUp(self):
        ...
        self.request_path = '/v1/projects/{0.slug}/data_types/{0.data_types[0].slug}/mzxml_files'.format(
            self.project
        )
        self.email = self.current_session.tokenized_user['email']
        self.headers = {'Authorization': 'Bearer ' + self.current_session.access_token}
        self.mzxml_file_path = os.path.abspath(
            'src/tests/support/fixtures/mzxml_files/sample.mzXML'
        )
        ...
        self.destination = os.path.join(
            current_app.config['MZXML_FILES_UPLOAD_FOLDER'],
            'projects',
            self.project.slug,
            'data_types',
            self.data_type.slug,
    ...
    def tearDown(self):
        ...
        shutil.rmtree(
            os.path.join(current_app.config['MZXML_FILES_UPLOAD_FOLDER'], 'projects'),
            ignore_errors=True,
        )

    @freeze_time('2020-06-02 08:57:53')
    def test_when_user_access_token_with_data_type_slug_and_multiple_mzxml_file_are_valid(
        self,
    ):
        params = {
            'mzxml_file_0': self.mzxml_file_stores[0],
        ...
        with self.client as rdbclient:
            ...
            for index in range(len(outcomes)):
                ...
                self.assertTrue(
                    outcome['name'] == [self.mzxml_file_name, 'another_sample'][index]
                )
                ...
                data_type = DataType.query.filter_by(
                    slug=outcome['data_type_slug']
                ).first()
                mzxmls = data_type.mzxml_files
    ...
    def test_when_user_access_token_is_valid_and_mzxml_file_has_no_name(self):
        ...
        with self.client as rdbclient:
            response = rdbclient.put(
                self.request_path,
                headers=self.headers,
                data={'mzxml_file_0': mzxml_file_store},
                content_type='multipart/form-data',
            )
    ...
    @freeze_time('2020-06-02 08:57:53')
    def test_when_user_access_token_is_valid_and_mzxml_file_extension_is_not_an_allowed_data_format(
        self,
    ):
        mzxml_file_store = FileStorage(
            stream=open(self.mzxml_file_path, 'rb'),
    ...
    def test_when_user_access_token_is_valid_and_mzxml_file_extension_is_configured_in_data_type_data_formats(
        self,
    ):
        mzxml_file_store = FileStorage(
            stream=open(self.mzxml_file_path, 'rb'),
    ...
    def test_when_user_access_token_is_valid_and_none_existing_data_type_slug(self):
        with self.client as rdbclient:
            response = rdbclient.put(
                '/v1/projects/'
                + self.project.slug
                + '/data_types/none-existing-data_type/mzxml_file',
                headers=self.headers,
                data=self.params,
                content_type='multipart/form-data',
            )
