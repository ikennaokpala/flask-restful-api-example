...
from src.main.config.tasks.db import create, seed, drop
...
class TestCommands(BaseTestCase):
    ...
    def test_db_create_when_database_is_available(self):
        with self.app.app_context():
            with patch(
                'src.main.config.tasks.db.database_exists', return_value=True,
            ) as mocked_database_exists:
                with patch(
                    'src.main.config.tasks.db.drop_database', return_value=None,
                ) as mocked_drop_database:
                    with patch(
                        'src.main.config.tasks.db.create_database', return_value=None,
    ...
    def test_db_create_when_database_is_not_available(self):
        with self.app.app_context():
            with patch(
                'src.main.config.tasks.db.database_exists', return_value=False,
            ) as mocked_database_exists:
                with patch(
                    'src.main.config.tasks.db.drop_database', return_value=None,
                ) as mocked_drop_database:
