...
class TestUploadMetadataShipmentFileAndAssociateWithDataType(BaseTestCase):
    def setUp(self):
        ...
        self.test_request_path = (
            '/v1/projects/'
            + self.project.slug
            + '/data_types/'
            + str(self.data_type.slug)
            + '/metadata_shipments'
        )
        self.email = self.current_session.tokenized_user['email']
        self.headers = {'Authorization': 'Bearer ' + self.current_session.access_token}
        self.metadata_shipment_path = os.path.abspath(
            'src/tests/support/fixtures/metadata_shipments/sample_lsarp_metadata_shipment.xlsx'
        )
        ...
        self.metadata_shipment_content = {
    ...
    def test_when_user_access_token_with_data_type_slug_and_multiple_metadata_shipment_are_valid(
        self,
    ):
        params = {
            'metadata_shipment_0': self.metadata_shipment_stores[0],
            'metadata_shipment_1': self.metadata_shipment_stores[1],
        }
    ...
    @freeze_time('2020-06-02 08:57:53')
    def test_when_user_access_token_with_data_type_slug_and_metadata_shipment_are_valid(
        self,
    ):
        with self.client as rdbclient:
            response = rdbclient.put(
    ...
    @freeze_time('2020-06-02 08:57:53')
    def test_when_user_access_token_is_valid_and_metadata_shipment_is_not_expected(
        self,
    ):
        with self.client as rdbclient:
            response = rdbclient.put(
                self.test_request_path,
                headers=self.headers,
                data={'fake_metadata_shipment': None},
                content_type='multipart/form-data',
            )
    ...
    def test_when_user_access_token_is_valid_and_metadata_shipment_has_no_name(self):
        ...
        with self.client as rdbclient:
            response = rdbclient.put(
                self.test_request_path,
                headers=self.headers,
                data={'metadata_shipment_0': metadata_shipment_store},
                content_type='multipart/form-data',
            )
    ...
    @freeze_time('2020-06-02 08:57:53')
    def test_when_user_access_token_is_valid_and_metadata_shipment_extension_is_not_xlsx(
        self,
    ):
        metadata_shipment_store = FileStorage(
            stream=open(self.metadata_shipment_path, 'rb'),
    ...
    @patch.dict(
        current_app.config,
        {'DATA_FORMAT_FILE_EXTENSIONS': ['some_other_valid_data_format']},
    )
    def test_when_user_access_token_is_valid_and_metadata_shipment_extension_is_not_xlsx(
        self,
    ):
    ...
    @freeze_time('2020-06-02 08:57:53')
    def test_when_user_access_token_is_valid_and_metadata_shipment_file_with_wrong_column_names(
        self,
    ):
        metadata_shipment_path = os.path.abspath(
            'src/tests/support/fixtures/metadata_shipments/sample_lsarp_metadata_shipment_wrong_columns.xlsx'
        )
        metadata_shipment_store = FileStorage(
    ...
    @freeze_time('2020-06-02 08:57:53')
    def test_when_user_access_token_is_valid_and_metadata_shipment_extension_is_defined_in_data_type_data_formats(
        self,
    ):
        metadata_shipment_store = FileStorage(
            stream=open(self.metadata_shipment_path, 'rb'),
    ...
    def test_when_user_access_token_is_valid_and_none_existing_data_type(self):
        with self.client as rdbclient:
            response = rdbclient.put(
                '/v1/projects/'
                + self.project.slug
                + '/data_types/none-existing-data-type-slug/metadata_shipment',
                headers=self.headers,
                data=self.params,
                content_type='multipart/form-data',
            )
