...
class DataTypesDAO:
    ...
    def data_types_decorator(callable):
        data_types_info = make_dataclass(
            'DataTypesInfo', ['page', 'per_page', 'total', 'data_types']
        )

        def wrapper(*args, **kwargs):
            data_types = callable(*args, **kwargs).__dict__

            return data_types.pop('query') and data_types_info(
    ...
    def ranker(self):
        if self.direction == 'asc':
            return asc(DataType.created_at)
