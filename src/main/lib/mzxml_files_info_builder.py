...
from src.main.validators.mzxml_file_validator import MZXmlValidator
...
class MZXmlFilesInfoBuilder:
    def __init__(self, data_type, mzxml_files, validator=MZXmlValidator):
        ...
        self.destination = os.path.join(
            self.mzxml_files_projects_data_type_directory, self.data_type.slug
        )
        self.mzxml_files_key_prefix = current_app.config['MZXML_FILES_KEY_PREFIX']
        self.mzxml_file_info = make_dataclass(
            'ProjectDataTypeMZXml',
            [
                'mzxml_file',
                'filename',
                'name_extension',
