...
class Environment:
    ...
    TEST_DATA_COLLABORATORS = os.getenv(
        'TEST_DATA_COLLABORATORS',
        'dev@westgrid.ca,ikenna.okpala@computecanada.ca,patrick.mann@computecanada.ca,swacker@ucalgary.ca,snoskov@ucalgary.ca,ian.lewis2@ucalgary.ca,ian.percel@ucalgary.ca,fridman@ucalgary.ca',
    ).split(',')
...
class Development(Environment):
    DEBUG = True
    SQLALCHEMY_DATABASE_URI = (
        os.environ.get('LSARP_DATABASE_URL') + '/lsarp_development'
    )
...
environments = dict(development=Development, test=Test, production=Production)
